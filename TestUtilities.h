...
const int rowval = barrierNumber;

const int colval = coFlows;

int perbarriercounter[barrierNumber][coFlows];
...
time_t dequeuetimebegin;
time_t dequeuetimenow;
time_t dequeuetmedelta;

time_t enqueuetimebegin;
time_t enqueuetimenow;
time_t enqueuetimedelta;
...
static struct sk_buff *fq_dequeue(struct Qdisc *sch, struct fq_sched_data *q)
{
  ...
  struct fq_flow_head *head;
  struct sk_buff *skb;
  struct fq_flow *f, *coflow, *fcurrent, *temp;
  unsigned long rate;
  int dcounter = 0;
  int coflowcounter = 0;
  u32 plen;
...
if(rValue != 1)
  ...
  if(!dequeuebarrierflag)

  {


   dequeuetimebegin = time(NULL);
   dequeuebarrierflag = 1;


  }

  dequeuetimenow = time(NULL);

  dequeuetmedelta = dequeuetimenow - dequeuetimebegin;

  printf("timedelta is %d \n", dequeuetmedelta);

  if((barrier[dcounter]!= barriervalue   ) && (dequeuetmedelta < 3))

  {

   head->first = f->next;
    // * HEON: this is where we mainly expect it to move the new flows to the old flows list
    printf("MOVING: flow to OLD flow && iterating to NEXT flow due to wait \n\n");
  ...
  /*  //Dequeue if per barrier counter allows else flow goes to old flows set and packet is sent in the next barrier
    if(perbarriercounter[dcounter][rValue])
    {
    fq_dequeue_skb(sch, f, skb);
    perbarriercounter[dcounter][rValue]--;
    }
    else
    {
    ...
    perbarriercounter[dcounter+1][rValue]++;
...
/*    int lengthOfarray = 0;
  ...
  memset(perbarriercounter, 0, sizeof(perbarriercounter[0][0])*barrierNumber*coFlows);
  ...
  if(!barrierflag)
   ...
   enqueuetimenow = time(NULL);

   enqueuetimedelta = enqueuetimenow - enqueuetimebegin;

   printf("enqueue time delta is %d \n", enqueuetimedelta);

  if (pValue != -1 )

	  {

	  if(enqueuetimedelta < 3)
  {

	printf("in If socket hash value is %u \n", f->socket_hash);
    barrier[barriercounter_flow] =
        barrier[barriercounter_flow] | 1 << pValue;
        perbarriercounter[barriercounter_flow][pValue]++;

    fq_skb_cb(skb)->time_to_send = skb->tstamp;

     printf("in if barrier value is %d \n", barrier[barriercounter_flow]);

	 printf("in if barrier counter value  is %d \n", barriercounter_flow );

	 printf("per barrier counter  is %d \n", perbarriercounter[barriercounter_flow][pValue]);


  }

  else
  {		  printf("in else socket hash value is %u \n", f->socket_hash);
	      barrierflag = 0;

	      barriercounter_flow++;

	      enqueuetimebegin = time(NULL);// barrier counter has been increased
		  barrier[barriercounter_flow] =
        barrier[barriercounter_flow] | 1 << pValue;
        perbarriercounter[barriercounter_flow][pValue]++;

	    printf("in else barrier value is %d \n", barrier[barriercounter_flow]);

	    printf("in else barrier counter value  is %d \n", barriercounter_flow);
