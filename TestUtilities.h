...
begin:
  ...
  if(flipflag)
  {
  printf("flipflag has been set \n");
  head = &q->co_flows;
  }
  else
  {
  printf("flipflag has not been set \n");
  head = &q->new_flows;
  }
  if (!head->first) {
    printf("In new-flows \n");
 ...
 if((rValue != -1) && (head != &q->co_flows))
 {
  ...
  printf("promote clows \n ");

  Promotecoflows(&q->old_flows, &q->new_flows, &q->co_flows, f, coflow,
                   pFlowid, lengthOfarray);
   printf("ucounter value before increment : %u \n ", ucounter);

   ucounter =3;
   
   flipflag =1;

   printf("ucounter value after increment : %u \n ", ucounter);
 ...
 if ( (ucounter == 3) && ((head != &q->co_flows)) )
  {
  printf("breach occured flip flag is set\n");
...
*/
  ...
  printf("breach finished flip flag has to be reset\n");
  ...
  if((flipflag) && (head == &q->co_flows))
  {
   ...
   printf("ucounter value before decrement : %u \n ", ucounter);
   ucounter--;
   printf("ucounter value after decrement : %u \n ", ucounter);
